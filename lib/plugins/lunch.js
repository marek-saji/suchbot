...
{
    return Promise.all(config.lunchSpreadsheets.map(cfg => {
        return new Promise((resolve, reject) => {
            googleSpreadsheets(
                {
                    key: cfg.id,
    ...
    return connect()
        .then(spreadsheets => {
            const promises = spreadsheets.map((spreadsheet, spreadsheetIdx) => {
                const spreadsheetCfg = config.lunchSpreadsheets[spreadsheetIdx];
                let worksheet;
                ...
                if (! worksheet)
                {
                    throw new Error('Worksheet with id=' + spreadsheetCfg.sheetId + ' not found.');
                }
...
{
    let now = new Date();
    let ms = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 13, 50, 0, 0) - now; // FIXME Read from spreadsheet
...
l18n.forEach(texts => {
    var urls = config.lunchSpreadsheets.map(
