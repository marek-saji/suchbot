...
async function getLunches (matchUserByName)
{
    ...
    const lunches = await getLunchesFromAllSheets(matchUserByName);
...
async function getLunch (matchUserByName, nick)
{
    const lunches = await getLunches(matchUserByName);

    const lunch = lunches.get(nick);
    if (! lunch && null !== lunch)
    {
        throw undefined;
    }
...
async function onOpen (backend)
{
    ...
    {
        // fill up the cache
        const lunches = await getLunches(backend.matchUserByName.bind(backend));
        if (lunches.size)
        {
