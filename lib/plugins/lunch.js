    ...
    try
    {
        ...
        if (lunches.size)
        {
            ...
            lunches.forEach(lunch => {
                if (! lunch.nick)
                {
                    log(`- (unknown user), ${lunch.userName}:`, lunch.value);
                }
                else
                {
                    log(`- @${lunch.nick}, ${lunch.userName}:`, lunch.value);
                }
            });
        }
        else
...
function onMessage (event)
{
    ...
    const nickToUserNamesMap = event.getNickToUserNamesMap();
...
{
    const nickToUserNamesMap = backend.getNickToUserNamesMap();
    const lunches = await getLunches(nickToUserNamesMap);
    lunches.forEach(lunch => {
        const {nick, value} = lunch;
        if (nick && value)
        {
