    ...
    return connect()
        .then(spreadsheets => {
            const promises = spreadsheets.map((spreadsheet, spreadsheetIdx) => {
                ...
                // FIXME Check whether range & nicksRange is not out of range
                //       with worksheet.rowCount & worksheet.colCount,
                //       because library does not validate that and just fails
                //       on 400 Bad Request
                return getLunchSheetRange(spreadsheetCfg.nicksRange)
                    ...
                    .then((nicksAndLunchesCells) => {
                        ...
                        row =
                            parseInt(spreadsheetCfg.range.match(/^R([0-9]+)/)[1], 10)
                            + ((new Date()).getDay() - 1) % 7;
