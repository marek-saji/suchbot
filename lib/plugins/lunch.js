...
async function getLunchesFromSheet (matchUserByName, sheets, spreadsheetCfg)
{
    // Mon=0 … Sun=6
    ...
    return names.reduce(
        async (carry, name, nameIdx) => {
            ...
            if (allLunches[dayIdx][nameIdx] === undefined)
            {
                allLunches[dayIdx][nameIdx] = null;
            }
            else if (ignoredValuesRegExp.test(allLunches[dayIdx][nameIdx]))
            {
                allLunches[dayIdx][nameIdx] = null;
            }

            let nick = await matchUserByName(name);

            if (!nick)
            {
                nick = `${name} (user not found)`;
            }

            return carry.then(
                lunches => {
                    lunches.set(nick, allLunches[dayIdx][nameIdx]);
                    return lunches;
                }
            );
        },
...
async function getLunchesFromAllSheets (matchUserByName)
{
...
function getLunches (matchUserByName)
{
    ...
    {
        return getLunchesFromAllSheets(matchUserByName)
            .then((lunches) => {
...
function getLunch (matchUserByName, nick)
{
    return getLunches(matchUserByName).then((lunches) => {
...
function onOpen (backend)
{
    ...
    getLunches(backend.matchUserByName.bind(backend))
        .then(lunches => {
            if (lunches.size)
            {
...
function onMessage (event)
{
    ...
    l18n.forEach(texts => {
        ...
        {
            handled = true;
            getLunch(event.matchUserByName, event.userName)
                .then(lunch => {
                    if (!lunch)
                    {
...
{
    return getLunches(backend.matchUserByName.bind(backend))
        .then(lunches => {
            lunches.forEach((lunch, nick) => {
                if (lunch)
                {
