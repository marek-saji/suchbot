...
const l18n = [
    {
        ...
        answers: {
            lunch: [
                '{name} ma zamówione {lunch}',
                '{lunch} (o ile {name} to ty)',
            ],
            noLunch: [
...
async function getLunchesFromAllSheets ()
{
    ...
    {
        sheetLunches.forEach((lunch, name) => {
            if (! lunches.has(name))
            {
                lunches.set(name, lunch);
            }
            else
            {
                lunches.set(name, [lunches.get(name), lunch].join('\n'));
            }
        });
    }
    return lunches;
...
async function getLunches ()
{
    ...
    const lunches = await getLunchesFromAllSheets();
...
async function getLunch (nickToUserNamesMap, userName)
{
    const lunches = await getLunches();
    const lunchesUserNames = Array.from(lunches.values())
        .map(lunch => lunch.userName);
    const lunchUserName = findUserNameInLunches(
        lunchesUserNames,
        nickToUserNamesMap,
        userName
    );
    ...
    return lunches.get(lunchUserName);
}

function findUserNameInLunches (lunchesUserNames, nickToUserNamesMap, userName)
{
    const userNameFromLunchesToUserNameMap = mapNamesToNicks(
        nickToUserNamesMap,
        lunchesUserNames
    );
    for (
        const [cUserNameFromLunches, cUserName]
        of
        userNameFromLunchesToUserNameMap
    )
    {
        if (cUserName === userName)
        {
            return cUserNameFromLunches;
        }
    }
    return undefined;
}
    ...
    try
    {
        ...
        if (lunches.size)
        {
            ...
            lunches.forEach(lunch => {
                log(`- ${lunch.userName}:`, lunch.value);
            });
        }
        else
        {
        ...
        if (texts.lunchQuestionRegExp.test(event.normalizedText))
        {
            ...
            getLunch(nickToUserNamesMap, event.userName)
                .then(lunch => {
                    ...
                    event.respond(texts.answers.lunch.map(
                        f => f.replace('{lunch}', lunch.value).replace('{name}', lunch.userName)
                    ));
                })
                .catch(error => {
                    logger.error(error);
